...
class TestPackageStructure:
    ...
    def test_entry_point_exists(self, project_root: Path) -> None:
        ...
        assert "yapcli = " in content, "Entry point 'yapcli' not found in pyproject.toml"
    ...
    def test_package_data_configured(self, project_root: Path) -> None:
        ...
        content = pyproject.read_text()
        assert "[tool.setuptools.package-data]" in content, "package-data section not found"
        assert 'yapcli = ["frontend/build/**/*"]' in content, "Frontend build not included in package-data"
...
class TestPackageBuild:
    ...
    def test_sdist_build(self, project_root: Path, frontend_build: Path, build_dir: Path) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert frontend_in_sdist.exists(), "Frontend build not included in source distribution"
            assert (frontend_in_sdist / "index.html").exists(), "index.html not in source distribution"

    def test_wheel_build(self, project_root: Path, frontend_build: Path, build_dir: Path) -> None:
...
class TestInstalledPackage:
    ...
    def test_yapcli_command_exists(self, installed_package: Path, venv_dir: Path) -> None:
    ...
    def test_frontend_files_installed(self, installed_package: Path, venv_dir: Path) -> None:
        """Test that frontend files are installed with the package."""
        # Find site-packages directory
        result = subprocess.run(
            [str(installed_package), "-c", "import yapcli; import os; print(os.path.dirname(yapcli.__file__))"],
            capture_output=True,
            text=True,
        )
        ...
        assert (frontend_build / "index.html").exists(), "index.html not found in installed frontend"
